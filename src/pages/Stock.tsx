        ...
        <Card title="Frames (meters available)">
          <StockTable
            ...
            onAdd={() =>
              setCatalog((prev) => {
                const stock = prev.stock || {};
                const next: StockFrame = { profileId: "", metersAvailable: 0, minThreshold: 0 };
...
function StockTable({
  ...
  return (
    <div className="space-y-2">
      ...
      <button onClick={onAdd} className="rounded border px-3 py-2 text-sm hover:bg-black hover:text-white">
        Add
      </button>
    </div>
  );
