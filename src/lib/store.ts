...
export type Frame = { id: string; name: string; pricePerMeter: number; faceWidthCm: number; color?: string }
export type Mat = { id: string; name: string; pricePerSqM: number; color?: string }
export type Glazing = { id: string; name: string; pricePerSqM: number }
export type PrintingMaterial = { id: string; name: string; pricePerSqM: number }

export type StockFrame = { profileId: string; metersAvailable: number; minThreshold?: number }
export type StockSheet = { id: string; type: 'mat' | 'glazing' | 'backer'; sku?: string; widthCm: number; heightCm: number; qty: number; minThreshold?: number }
export type StockRoll = { materialId: string; widthCm: number; metersRemaining: number; minThreshold?: number }
...
function loadCatalog(): Catalog | null { return safeParse<Catalog>(localStorage.getItem(STORAGE_KEY)) }
function saveCatalog(catalog: Catalog) { localStorage.setItem(STORAGE_KEY, JSON.stringify(catalog)) }

function uid() { return Math.random().toString(36).slice(2, 10) }

const defaultCatalog: Catalog = {
  frames: [
    { id: 'fr1', name: 'Black Classic 20mm', pricePerMeter: 120, faceWidthCm: 2, color: '#111' },
    { id: 'fr2', name: 'Oak 30mm', pricePerMeter: 180, faceWidthCm: 3, color: '#b88a58' },
  ],
  mats: [
    { id: 'mat0', name: 'No mat', pricePerSqM: 0, color: 'transparent' },
    { id: 'mat1', name: 'Bright White', pricePerSqM: 220, color: '#F8F8F4' },
    { id: 'mat2', name: 'Museum Off-white', pricePerSqM: 280, color: '#EFEDE6' },
    { id: 'mat3', name: 'Charcoal', pricePerSqM: 260, color: '#3a3a3a' },
  ],
  glazing: [
...
function ensureMat0(mats: Mat[]): Mat[] {
  return mats.some(m => m.id === 'mat0') ? mats : [{ id: 'mat0', name: 'No mat', pricePerSqM: 0, color: 'transparent' }, ...mats]
}

function migrateCatalog(oldCat: Partial<Catalog> | null): Catalog {
  if (!oldCat) return defaultCatalog
  const frames = Array.isArray(oldCat.frames) && oldCat.frames.length ? oldCat.frames : defaultCatalog.frames
  const mats = ensureMat0(Array.isArray(oldCat.mats) && oldCat.mats.length ? oldCat.mats : defaultCatalog.mats)
  const glazing = Array.isArray(oldCat.glazing) && oldCat.glazing.length ? oldCat.glazing : defaultCatalog.glazing
...
export function newFrame(partial?: Partial<Frame>): Frame {
  return { id: partial?.id || `fr_${uid()}`, name: partial?.name || 'New Frame', pricePerMeter: partial?.pricePerMeter ?? 0, faceWidthCm: partial?.faceWidthCm ?? 2, color: partial?.color ?? '#111' }
}
export function newMat(partial?: Partial<Mat>): Mat {
  return { id: partial?.id || `mat_${uid()}`, name: partial?.name || 'New Mat', pricePerSqM: partial?.pricePerSqM ?? 0, color: partial?.color ?? '#EEEEEE' }
}
export function newGlazing(partial?: Partial<Glazing>): Glazing {
  return { id: partial?.id || `gl_${uid()}`, name: partial?.name || 'New Glazing', pricePerSqM: partial?.pricePerSqM ?? 0 }
}
export function newPrintingMaterial(partial?: Partial<PrintingMaterial>): PrintingMaterial {
  return { id: partial?.id || `pm_${uid()}`, name: partial?.name || 'New Printing Material', pricePerSqM: partial?.pricePerSqM ?? 0 }
}
